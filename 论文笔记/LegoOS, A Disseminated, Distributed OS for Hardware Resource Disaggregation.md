...
7. 这篇论文的缺点：单个进程仍然禁锢在一个 `pCompenent` 中，论文中说是由于无法保证跨组件的Cache的一致性（如果要保证的话需要很多的带宽）。这里向勇老师提供的一个思路：在用户程序和编译器以及操作系统上做优化：识别同一个进程中使用数据相对独立的进程，将它们放到不同的组件中运行，这样由于共用的数据较少，为保证一致性所占用的带宽也能很大程度上减小。