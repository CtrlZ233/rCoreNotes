...
6. 这篇论文的优点：设计非常符合模块化原则，各个硬件独自实现自己资源的调度算法，再由全局调度算法来做粗粒度调度。对应到进程/线程/协程调度的具体应用，是否可以同样设计：把具体的调度算法和数据结构分离，数据结构为调度算法实现几个通用的接口（存、取、访问等），调度算法通过这几个接口再自己实现更加复杂的调度。
7. 这篇论文的缺点：单个进程仍然禁锢在一个 `pCompenent` 中，论文中说是由于无法保证跨组件的Cache的一致性（如果要保证的话需要很多的带宽）。这里向勇老师提供的一个思路：在用户程序和编译器以及操作系统上做优化：识别同一个进程中使用数据相对独立的线程，将它们放到不同的组件中运行，这样由于共用的数据较少，为保证一致性所占用的带宽也能很大程度上减小。